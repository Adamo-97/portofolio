...
const CategoryFolder: React.FC<CategoryFolderProps> = ({ category, projects, index, isOpen, onToggle }) => {
  ...
  return (
    ...
    >
      ...
      <motion.button
        onClick={onToggle}
        className="relative w-full rounded-lg border border-white/10 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:bg-white/[0.08] group cursor-pointer"
        whileTap={{ scale: 0.98 }}
        style={{
          background: isOpen ? `${colors.accent}15` : 'rgba(255,255,255,0.04)',
          borderTopColor: colors.accent,
          borderTopWidth: '3px',
          boxShadow: isOpen ? `0 0 30px ${colors.glow}, 0 0 60px ${colors.glow}` : 'none',
        }}
      >
        ...
        <div
          ...
          style={{
            background: `linear-gradient(180deg, ${colors.glow} 0%, transparent 60%)`,
          }}
        />

        {/* Content - Compact Vertical */}
        <div className="relative flex flex-col items-center px-3 py-3 sm:px-4 sm:py-4 lg:px-5 lg:py-5 gap-1 sm:gap-1.5">
          {/* Category Name & Count */}
          <div className="flex flex-col items-center gap-0.5 sm:gap-1">
            <h3 className="text-[13px] sm:text-[15px] lg:text-[17px] font-semibold text-white tracking-tight text-center">
              {category}
            </h3>
